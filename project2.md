Welcome to **TreeLife**, your guide to learning about the beauty, diversity, and importance of trees around the world. 

# About Us
At **TreeLife**, we're passionate about forests and green living. 
Our mission is to: 

🞗 Educate people about the different types of trees.

🞗 Promote sustainable forestry.

🞗 Encourage reforestation projects. 
> "The best time to plant a tree was 20 years ago. The seconnd best time is now." - *Chinese Proverb* 

# Featured Trees 
### Oak Tree
**Sciencetific Name:** *Quercus robur* 

Known for its strength and longevity, the oak is a symbol of endurance.
![alttext](https://treenewal.com/wp-content/uploads/2020/11/oak-tree-care.png)

### Pine Tree 
**Scientific Name:** *Pinus* 

Evergreen and aromantic, pine trees thrive in colder regions.
![alttext](https://images.squarespace-cdn.com/content/v1/5982fa4fe58c62cb28091fa4/1569233367092-HTN9E2NOCLHMOVSPHOJ2/image-asset.jpeg)


## Tree Identification Tool 

You can use this simple **Javascript** function to identify a tree by its characteristics: 

  function identify_tree(leaf-shape, region){                                    
  if (leaf_shape ## "broad" && ## "cold") {                                      
   return "Pine Tree"                                                            
  } else if(**leaf_shape** ## "broad" && **region** ## "temprerature) {                                                                  
   return "Oak Tree"                                                           
  } else {                                                                       
  return "Unknown Tree"                                                          
                                                                                     }                                                                             
   **console**.log(indetify_tree("needle", "cold") )
   